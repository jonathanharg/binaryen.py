...
def copy_header():
    ...
    if not os.path.isfile(wasm_def_path):
        ...
        wasm_def_source = os.path.join(
            libbinaryen_dir, "./binaryen/src/wasm-delegations.def"
        )
...
def compile_binaryen():
    ...
    version_file = open(
        os.path.join(libbinaryen_dir, "version.txt"), encoding="utf-8"
    ).read()
    ...
    comiler_args = [
        ...
        "-B",
        target_dir,
        f"-DCMAKE_INSTALL_PREFIX={target_dir}/install",
